...
`internal/cluster` plus a thin public surface (`OpenCluster`, `ClusterConfig`).
This keeps the single-binary, minimal-dependency identity intact for embedded
users. CLAUDE.md and DESIGN §12 are updated to reposition distribution as a **v2,
...
A spike (`internal/cluster/spike_test.go`) confirmed, under `-race` and with
`CGO_ENABLED=0`:
...
- New direct dependencies: `hashicorp/raft`, `hashicorp/raft-boltdb/v2` (the latter
  ...
  Linked only when `OpenCluster` is imported.
- The public API gains `OpenCluster`/`ClusterConfig` and a per-query read
  consistency option; `Open`/`Query` semantics for embedded use are unchanged.
